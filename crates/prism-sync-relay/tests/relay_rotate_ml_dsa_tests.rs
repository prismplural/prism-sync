...
fn build_rotation_request(
    ...
    let new_sk = keys.device_secret.ml_dsa_65_keypair_v(device_id, new_generation).unwrap();
...
async fn test_valid_rotation_succeeds() {
    ...
    let device =
        db.with_read_conn(|conn| db::get_device(conn, &sync_id, &device_id)).unwrap().unwrap();
...
async fn test_invalid_proof_returns_400() {
    ...
    let new_secret = DeviceSecret::generate();
    let (mut body_bytes, _new_pk) = build_rotation_request(&device_id, &keys, &new_secret, 0, 1);
...
async fn test_wrong_device_id_returns_403() {
    ...
    assert_eq!(resp.status(), 403, "rotating another device's key should be forbidden");
}
