...
async fn test_current_key_passes() {
    ...
    assert!(
        ...
        "current key should pass: {status}"
    );
}
...
async fn test_grace_key_passes_during_grace_period() {
    ...
    assert!(
        (200..300).contains(&status),
        "grace key should pass during grace period: {status}"
    );
}
...
async fn test_expired_grace_key_fails() {
    ...
    assert_eq!(
        status, 401,
        "expired grace key should be rejected: {status}"
    );
}
...
async fn test_unknown_key_fails() {
    ...
    assert_eq!(
        status, 401,
        "unknown key should be rejected: {status}"
    );
}
