...
mod tests {
    ...
    fn tampered_old_signs_new_fails() {
        ...
        assert!(
            proof.verify(&ed25519.public_key_bytes(), &old_ml_dsa.public_key_bytes()).is_err(),
    ...
    fn wrong_ed25519_pk_fails() {
        ...
        assert!(
            proof
                .verify(&wrong_ed25519.public_key_bytes(), &old_ml_dsa.public_key_bytes())
                .is_err(),
    ...
    fn wrong_old_ml_dsa_pk_fails() {
        ...
        assert!(
            proof.verify(&ed25519.public_key_bytes(), &wrong_ml_dsa.public_key_bytes()).is_err(),
    ...
    fn generation_rollback_rejected() {
        ...
        assert!(result.is_err(), "new_gen < old_gen should be rejected");
    }
